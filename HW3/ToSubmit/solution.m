...
% !!! Soluton 3 failed, a big bug is identified on the algorithms that
% procudes the finite difference matrix. 

% !!! Solution 4 Failed, I might have swapped the partial x operator matrix
% with the partial y operation matrix. 
...
function [A1, A2, A3] = Problem1()
    ...
    PartialX2d = kron(P2, eye(N));
    PartialX2d = PartialX2d./(2*DeltaX);

    PartialY2d = kron(eye(N), P2);
    PartialY2d = PartialY2d./(2*DeltaX);

    A1 = Laplacian;
    A2 = PartialX2d;
    figure; imagesc(A2); colorbar; title("A2 Partial x");
    A3 = PartialY2d;
    figure; imagesc(A3); colorbar; title("A3 Partial y");
...
function [A4, A5] = Problem2(Fmat, permvec)
    ...
    figure; imshow(abs(FmatDecrypted)./max(abs(FmatDecrypted)));
    A4 = abs(FmatDecrypted);
    A5 = abs(ifft2(ifftshift(FmatDecrypted)));
    figure; imshow(abs(A5)./max(abs(A5)));
